...
func (shellSurface) Screen(m *Model) string {
	...
	title := termTitleStyle.Render("SESSION // " + m.shell.HostName())
	term := termPanelFocusStyle.Width(termW - 2).Height(panelH - 2).
		Render(title + "\n" + m.shellVP.View())
	quest := termPanelStyle.Width(questW - 2).Height(panelH - 2).
		Render(m.questPanel())
