	...
	deck := engine.NewEntity("deck", "the deck", "rig", "computer", "terminal").With(
		...
		hacking.Deck{
			...
			Objectives: []hacking.Objective{
				{Flag: "heard_whisper", Text: "trace the whisper in the dead code"},
				{Flag: "got_sun_fragment", Text: "pull whatever 'sun' data is still out there"},
			},
		},
	)
	...
	return map[string]*hacking.Host{
		...
		"sunfarm.arc": {
			...
			Procs: []*hacking.Process{
				{PID: 27, Name: "whisperd", OnKill: "whisper_silenced"},
			},
		},
	}
}
